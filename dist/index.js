...
export default class VDate extends Date {
     ...
     */
    getTimeStamptInSecond(UTC = true) {
        if (UTC)
     ...
     */
    formatDateTime(reverse = false, ampm = false) {
        if (reverse) {
     ...
     * @return String -
     */
    static countDown(targetTime, countDownLimit = 432000, includeHour = false, includeMinutes = false, now = new VDate()) {
     ...
     * @returns Number
     */
    static getDaysInMonth(year = new VDate().getFullYear(), month = new VDate().getMonth()) {
     ...
     * @returns Number
     */
    static getDayOfYear(date = new VDate()) {
        let today = date;
